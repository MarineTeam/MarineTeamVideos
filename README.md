...
| `/api/video-invite` | GET / POST | Per-video private access list ("invite as a list", like a private YouTube video). GET `?videoId=` returns the current list with each member's live status. POST `{videoId, videoTitle, emails, hours, watermark, notify}` adds any emails not already on the list — each becomes a normal share (same as `/api/share`); unless `notify` is explicitly `false`, each new addition also gets the usual notification email (a Drive/YouTube-style "don't send an email" option — the link is still live either way). Emails already on the list are left untouched (no new record, no re-sent email) |
