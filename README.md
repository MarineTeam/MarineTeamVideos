...
- Each video also has a **Private list** button for YouTube-style private-video sharing: a persistent, editable invite list you maintain over time instead of one-off links. Add emails and only the ones that are actually new to that video's list get created as a share and emailed — re-adding someone already on the list does nothing. Remove someone and their link is revoked immediately; if you invite that email again later, it's treated as a fresh invite. This sits on top of the same share/bundle machinery as regular Share/Bulk Share, it's just a different way of managing who currently has access to one video.
...
| `/api/video-invite` | GET / POST | Per-video private access list ("invite as a list", like a private YouTube video). GET `?videoId=` returns the current list with each member's live status. POST `{videoId, videoTitle, emails, hours, watermark}` adds any emails not already on the list — each becomes a normal share (same as `/api/share`) and gets the usual notification email; emails already on the list are left untouched (no new record, no re-sent email) |
