...
// Sorted-set operations, used for the created-at ordered share index that
// lets the admin listing read ONE page instead of every record
// (lib/shareQuery.js). Kept alongside the plain set rather than replacing
// it: the plain index is what cleanup, revoke-permanent and every filtered
// query still use, and renaming or retiring it would be exactly the kind of
// silent migration that orphaned records in 30ecd7f.
export async function kvZadd(key, score, member) {
  return kvFetch(
    `/zadd/${encodeURIComponent(key)}/${encodeURIComponent(score)}/${encodeURIComponent(member)}`
  );
}

export async function kvZrem(key, member) {
  return kvFetch(`/zrem/${encodeURIComponent(key)}/${encodeURIComponent(member)}`);
}

export async function kvZcard(key) {
  const r = await kvFetch(`/zcard/${encodeURIComponent(key)}`);
  return Number(r.result) || 0;
}

// Inclusive rank range, highest score first (REV) — i.e. newest first for a
// createdAt-scored set. Ranks, not scores, so paging is start/stop
// arithmetic with no cursor to carry.
export async function kvZrangeRev(key, start, stop) {
  const r = await kvFetch(
    `/zrange/${encodeURIComponent(key)}/${encodeURIComponent(start)}/${encodeURIComponent(stop)}/REV`
  );
  return r.result || [];
}
