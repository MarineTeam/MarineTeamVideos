...
import { kvGet, kvSet, kvSadd, kvZadd } from "./kv";
...
// A second, ORDERED index: the same tokens scored by createdAt, so the admin
// listing can read one page newest-first instead of fetching every record to
// sort them (lib/shareQuery.js). Purely additive — SHARE_INDEX_KEY above
// keeps its name, its shape and every call site, and a deployment that never
// backfills this one simply falls back to the old full read.
export const SHARE_BY_CREATED_KEY = "bunnyshare-by-created";
...
// Trims an admin-supplied note and caps its length, so a stray paste can't
// bloat every KV record or the emails built from them. Returns "" for
// anything empty, which callers treat as "no note".
const NOTE_MAX_LENGTH = 500;
export function normalizeNote(note) {
  return String(note || "").trim().slice(0, NOTE_MAX_LENGTH);
}
...
export async function createShareRecord({ videoId, videoTitle, email, hours, siteUrl, watermark, source, maxViews, note }) {
  ...
  const record = {
    ...
    // How this record was created: "invite" for the persistent per-video
    // Private list (lib/invites.js), absent for a normal Share/Bulk Share
    // call. Additive — absent on records made before this existed, which the
    // admin table reads as "Share" (see pages/index.js). There is no
    // "bulk" value: Bulk Share is still just a normal share from the
    // recipient's/record's point of view, one record per video either way.
    ...(source === "invite" ? { source } : {}),
    // Optional cap on authorized page renders, enforced in
    // pages/watch/[token].js alongside revoked/expired. Additive and stored
    // ONLY when a positive integer was supplied — absent means "no view
    // limit", which is exactly how every record made before this existed
    // behaves. Never write 0: that would read as a share nobody may open.
    ...(Number.isInteger(Number(maxViews)) && Number(maxViews) > 0
      ? { maxViews: Number(maxViews) }
      : {}),
    // Optional short note from the admin, shown in the notification email
    // and in the admin table. Stored raw and escaped at every render point
    // (lib/mailer.js escapeHtml for email, React for the admin table) —
    // never pre-escaped here, so the stored value stays the literal text
    // the admin typed.
  ...
  try {
    ...
    await kvZadd(SHARE_BY_CREATED_KEY, record.createdAt, token);
  } catch (err) {
